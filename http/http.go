...
func SendGETRequest_v2(uri string) (*http.Response, error){
	// Create http request
	req, err := http.NewRequest("GET", uri, nil)
	if err != nil {
		fmt.Println("Error creating request:", err)
	...
	// set header
	req.Header.Set("Accept", "vdn.dac.v2")
	req.Header.Set("Content-Type", "application/json")
...
func SendPUTRequest_v2(uri string, jsonBody string) (*http.Response, error) {
    // Create http request
    req, err := http.NewRequest("PUT", uri, strings.NewReader(jsonBody))
    if err != nil {
        fmt.Println("Error creating request:", err)
